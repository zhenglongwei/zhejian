...
const AUTHORIZATION_CONSENT = {
  ...
  case_public: {
    ...
    version: LEGAL_VERSION,
    text:
      '我已阅读《公开案例与隐私说明》，核对公开内容与脱敏效果，确认发布到公开网站',
  },
  desensitize_confirm: {
    authType: 'desensitize_confirm',
    version: LEGAL_VERSION,
    text: '本人已逐张核对脱敏效果，确认发布到公开网站',
  },
  case_revoke: {
    authType: 'case_revoke',
    version: LEGAL_VERSION,
    text: '我确认撤回发布，知晓公开网站上的案例将下架',
  },
  merchant_onboard: {
...
const COMPLIANCE_COPY = {
  price: '到店检测后确定。',
  casePrice:
    '车主已发布到公开网站的案例展示当时方案报价；其余案例价格为系统参考区间，实际费用以门店检测为准。',
  authorizedCaseFixed:
    '本案例为车主发布到公开网站的内容，价格为当时方案报价，不构成线上报价承诺。',
  accident: '到店检测后确定。',
  history: '价格仅供参考，不代表本页报价或最终成交价。',
  authorize:
    '发布到公开网站后，你的服务相册经脱敏处理可供其他车主参考。公开版不展示完整车牌、手机号等信息，请你核对后再确认。展示前须完成审核；你可随时撤回。',
  partRisk:
  ...
  reviewImageTip:
    '配图默认仅你与门店可见；若随案例发布到公开网站，将自动脱敏后展示；建议尽量避免完整车牌、人脸、证件。',
  reportImageTip: '截图将随举报一并提交，仅用于核实处理。请勿上传与举报无关的内容。',
  /** @deprecated 使用 reviewImageTip 或 reportImageTip */
  reviewUpload:
    '配图默认仅你与门店可见。勾选公开展示后将自动脱敏，并在公开案例中展示；建议尽量避免完整车牌、人脸、证件。',
  consult:
