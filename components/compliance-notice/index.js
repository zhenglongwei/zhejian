const COPY = {
  ...
  history: '价格仅供参考，不代表平台报价。',
  authorize:
    '授权公示后，你的服务相册经脱敏处理可供其他车主参考。公开版不展示完整车牌、手机号等信息，请你核对后再确认。公开展示须经平台审核。',
  partRisk:
    '该配件并非主机厂原厂件。请确认你已了解配件差异及后续年检、二手车检测、保险理赔或质保判断中可能被识别为非原厂件的风险。',
  reward: '评价奖励须符合平台规则，审核通过后发放，非「好评返现」。',
  desensitize:
    '以下为平台脱敏预览，公开展示仅使用脱敏图，不含完整车牌、人脸、手机号等信息。请逐张核对后再确认。',
  desensitizeGuide:
    '请先点击底部「一键 AI 脱敏」生成预览；手工打码将在后续版本开放。',
  desensitizePreMaskReview:
    '以下为平台自动脱敏预览，请逐张核对原图与脱敏图；如有遗漏可使用手工打码补充（即将开放）。',
  reviewUpload:
  ...
  platformDisplay:
    '本页内容由商家自行发布或经用户授权展示，仅供参考。平台不提供维修服务，不对价格、质量、效果作担保；实际方案与费用请与门店线下确认。',
}
...
Component({
  ...
  methods: {
    updateContent() {
      ...
      this.setData({
        content: text || COPY[type] || COPY.price,
        prewrap: type === 'partRisk',
      })
    },
  },
})
