...
const { persistApiCheck } = require('../services/geo-check-persist.service')
...
router.get('/geo-check/status', (req, res) => {
  ...
  return ok(res, {
    ...
    channels: {
      ...
      qwen: summary.channels.webQwenFallback.configured,
...
router.post('/geo-check', async (req, res) => {
  ...
  try {
    const data = await runGeoCheck({ companyName, city, industry })
    data.quota = quota

    // 落库：让主动体检的门店进榜单。落库失败不能影响本次返回。
    try {
      const persisted = await persistApiCheck(
