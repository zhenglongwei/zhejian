...
const GEO_PROBE_ENGINE_REGISTRY = [
  ...
  {
    id: 'hunyuan',
    label: '腾讯混元（TokenHub 联网）',
    defaultApiUrl: 'https://tokenhub.tencentmaas.com/v1/chat/completions',
    defaultModel: 'hy3-preview',
    webSearchMode: 'web_search_options',
    apiKeyEnvKeys: [
      'GEO_PROBE_HUNYUAN_API_KEY',
      'GEO_PROBE_YUANBAO_API_KEY',
      'TOKENHUB_API_KEY',
      'HUNYUAN_API_KEY',
    ],
    apiUrlEnvKey: 'GEO_PROBE_HUNYUAN_API_URL',
    modelEnvKey: 'GEO_PROBE_HUNYUAN_MODEL',
    modelEnvKeys: ['GEO_PROBE_HUNYUAN_MODEL', 'GEO_PROBE_YUANBAO_MODEL'],
    batchLimitEnvKey: 'GEO_PROBE_HUNYUAN_BATCH_LIMIT',
...
function resolveEngineRuntimeConfig(engineId, options = {}) {
  ...
  return {
    ...
    model: String(resolveFirstEnv(def.modelEnvKeys || [def.modelEnvKey]) || def.defaultModel).trim(),
    batchLimit,
    enableThinking,
    configured: false,
    removed: false,
