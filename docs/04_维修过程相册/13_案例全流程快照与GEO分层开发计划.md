> **修订 2026-08-17**：公开权改为商家发布，见 [`19_商家发布案例上网口径.md`](./19_商家发布案例上网口径.md)。快照冻结时机改为 **通知窗口结束且公开生效**；用户不再点发布。

# 案例全流程 · 快照分层与 GEO 提炼开发计划

> **生效日期**：2026-07-08  
> **状态**：定稿（**2026-07-27 案例审核收口**；**2026-08-11**：相册对车主全程开放，案例审不挡看相册）· 部分已开发  
...
**商家制造留档 → 确认完工锁定（后台可开始打码）→ 商家生成案例（等打完码再预览打码稿：摘要 / 本单 FAQ / 详情）→ 送审 → 平台案例人审 → 过审即快照冻结上线。** 不另做给 AI 搜的专题文章；店内类目仅为文件夹。下架不解锁商家。

### 1.2 角色分工

| 角色 | 职责 | 不做 |
| --- | --- | --- |
| **商家** | 创建相册、留档、**确认并完工**；**生成案例**并送审；驳回后改相册再完工再生成 | 不能在施工中公示；完工后不能改留档（驳回除外）；生成时不能加新图 |
| 用户（车主） | **全程可看服务相册**；公开后可下架；评价随店页以打码编号展示 | 不是公网发布者；不能改已冻结快照 |
| **辙见平台** | **案例审**：合法合规 + 脱敏风险；通知窗口；提炼层 GEO/SEO | **不改案例快照一字**；不把「已审核」写成质量背书 |
...
| D1c | **车主可见性（2026-08-11）** | 服务相册对关联车主**全程可查看**；案例审只审文稿，不挡相册 |
| D2 | 案例 vs 提炼层 | **公开生效瞬间**整包快照冻结；之后仅 **提炼层** 可改 |
| D3 | 已公示案例 | 快照 **一个字不能改**（含商家确认的要旨与本单 FAQ）；店内类目文件夹的案例数/链接可随上下线变化 |
| D4 | 撤回后再发布 | **新 snapshotVersion**；**不再**强制重审（案例审已过） |
| D5 | 商家套餐润色 / AI 润色 | **仅确认并完工前**；完工后不可再改案例正文 |
| D6 | GEO 大模型 | **见 §5 专业建议** |
| D7 | **单闸门 · 案例审** | 完工后脱敏结束再人审公示案例；**废止**相册完工合规阻塞审与发布后复审 |
| D8 | **预脱敏** | 完工后异步 pre-mask；**生成案例预览须打码就绪**（只展示打码图，失败不拿原图充数）；送审后脱敏结束才升入运营待审；失败可重试或驳回商家 |
| D9 | ~~发布后人审~~ | **废止** |
...
┌─────────────────────────────────────────────────────────────────────────┐
│ 小程序 · 商家端                                                          │
│  创建相册 → 六阶段留档 → 案例预览（打开即写顺）→ 确认并完工(completed)            │
│  商家锁定；车主仍可看相册                                               │
└───────────────────────────────┬─────────────────────────────────────────┘
...
| **车主可看相册** | 关联车主可打开服务相册详情 | **全程**（维修中 / 已完工 / 案例审中 / 驳回） |
| **解锁商家** | 商家可再编辑 | **仅**案例审驳回 |
...
**冻结时机**：公开生效时（通知窗口结束且未被阻止）写入 `content_json.snapshot`（新单）。存量车主点发布写入的快照不回写。
