...
> **生效日期**：2026-07-08  
> **状态**：定稿（产品原则已确认 · **2026-07-08 双闸门修订** · **2026-07-26 完工一审、废止发布后闸门 B**）· 部分已开发  
...
**商家制造留档 → 确认完工锁定 → 平台一审（原闸门 A）→ 通过后车主可看案例并发布（**不再过审**）→ 快照不可改 → 平台在快照外做 GEO 提炼。撤回公开下线且不解锁商家。**

### 1.2 角色分工

| 角色 | 职责 | 不做 |
| --- | --- | --- |
| **商家** | 创建相册、六阶段留档、**确认并完工**；一审驳回后改相册再送审；留档质量自负 | 不能替用户公示/撤回；**确认完工后**不能改留档（驳回除外）；撤回后也不能改 |
| **用户（车主）** | 一审通过前可看私密相册；通过后查看案例并决定是否发布；撤回、再发布 | 不对留档完整度负责；不能改已冻结快照；一审通过前不能看案例稿 |
| **辙见平台** | **一审**：完工合规（自动规则+抽检）；提炼层 GEO/SEO；收录与探测 | **不改案例快照一字**；**不再**在车主发布后再做人审闸门 B |

### 1.3 已确认的关键决策

| # | 决策 | 选项 |
| ---: | --- | --- |
| D1 | **商家留档锁定（2026-07-26）** | **确认并完工后**永久锁定；**仅**一审驳回可再编辑；**撤回不解锁** |
| D1b | **用户发布后锁定** | 与 D1 叠加；撤回仍锁定 |
| D2 | 案例 vs 提炼层 | **用户点发布瞬间**整包快照冻结；之后仅 **提炼层** 可改 |
| D3 | 已公示案例 | 快照 **一个字不能改**；FAQ/GEO/SEO/专题/聚合 FAQ **可随时改** |
| D4 | 撤回后再发布 | **新 snapshotVersion**；**不再**强制 B 重审（一审已过） |
| D5 | 商家套餐润色 / AI 润色 | **仅确认并完工前**；完工后不可再改案例正文 |
| D6 | GEO 大模型 | **见 §5 专业建议** |
| D7 | **单闸门（原 A）** | **一审**=完工后合法合规（自动规则+抽检）；**废止**发布后闸门 B |
| D8 | **预脱敏** | 完工后后端 pre-mask **不变**；不纳入一审人工审项 |
| D9 | ~~B 驳回~~ | **废止**（发布路径不再进人审） |
| D10 | **一审驳回** | 回到**商家**改相册、再确认完工/再送审 |
| D11 | **废止旧路径** | 「公示合规驳回 → 用户撤回 → 商家改相册」；以及「发布后再审」 |
...
| **解锁** | 商家可再次编辑留档 | **仅**一审驳回后商家改图；**撤回不解锁** |
