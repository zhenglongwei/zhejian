...
**商家制造留档 → 确认完工锁定 → 异步脱敏 → 平台案例人审（审公示案例稿，不挡车主看相册）→ 通过后案例稿/发布可开（**不再过审**）→ 快照不可改 → 平台在快照外做 GEO 提炼。撤回公开下线且不解锁商家。**
...
| **用户（车主）** | **全程可看服务相册**；案例审通过后可看案例稿并决定是否发布；撤回、再发布 | 不对留档完整度负责；不能改已冻结快照 |
