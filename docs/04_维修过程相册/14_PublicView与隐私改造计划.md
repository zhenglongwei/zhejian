...
> **版本**：2026-07-12 定稿 · **2026-07-22** 交叉修订（公域藏价 / 知识包见 `15_…`）  
> **状态**：实施中（Phase 1–4 代码已落地，P5 联调发版待完成）；**公域金额与教练引擎** 以 [`15_公域知识包与相册教练规则引擎.md`](./15_公域知识包与相册教练规则引擎.md) 为增量真源  
> **取代**：对话中「事后从全相册挑 4 张 + 每阶段双上传带 + 匿名公示档 + 原图私人分享」草案  
> **关联**：`00_Phase1_服务相册产品口径.md`、`15_公域知识包与相册教练规则引擎.md`、`13_案例全流程快照与GEO分层开发计划.md`、`00_开发计划.md` §7.8 / §7.11
...
| **价格诚实** | **私域**展示方案留档价；**公域藏价**（2026-07-22，见 `15_…`）；报价单图仅私密留档 |
...
| stage_3 方案 | 报价单仅留档；公示展示方案**文字与配件名**（**不展示金额**，2026-07-22） | `always_private`（图） |
...
- **公示引导**：缺 stage_3 方案说明时强提示「公示后将缺少方案说明」；**不再**因缺 `planAmount` 提示「公示后无报价」（公域本不展示金额）

### 2.5 增量定稿（2026-07-22 · 见 `15_…`）

- 公开配图默认约 **5～8** 关键帧；车主**可删不可加**；高风险**硬拦**作用于商家/系统选图（公开路径）；朋友圈不套用该硬拦。  
- 商家完工：LLM **仅文字**生成案例正文；图脱敏后嵌入。  
- 案例正文属商家；车主只读正文 + 可评价。  
- 商家编辑「相册教练」按服务类型规则引擎提示要拍/别拍/怎么写。  
- 发布确认语：鼓励式体验官邀请 + 一键下架承诺（见 `15_…` §13）。
...
- `media` = 公开配图包内 `visibility=public` 且 `gate=passed` 且 pre-mask 成功的图（目标默认约 **5～8**，软上限仍可至 30，见 `15_…`）  
- **软上限 30 张**（性能护栏）；Feed 封面仍 pick 1 张  
- stage_1 / stage_3 的图 **永不进入 media**  
- **公域不输出成交金额**（`planAmount` 不进公开读侧；2026-07-22）
...
| `albums.planAmount` | ❌（**公域藏价**，仅私域；2026-07-22） | ❌ |
| stage_3 报价单照片 | ❌ | ❌ |

**报价单不 OCR 上网**。可选 **商家端 OCR 辅助录入**（私密）：触发 OCR → 填入 `planParts` / `planAmount` → 商家核对保存；**配件名等可进公开文本，金额不进 PublicView**。
