...
> **生效日期**：2026-06-16  
> **状态**：定稿 · **A～C、E 已验收**；**D 部分完成**（规则图说 ✅，vision API 待 D06）；详见 §12 进度汇总  
...
| 3 | **质量闸门**：提交审核 **block**（证据链缺失）— **仅商家冷启动 / 非用户授权路径**；**用户授权公示** 提交与运营审核 **不 block**；运营对冷启动等 **warn**（完整度低）；**存量已上线案例不 retroactive block** |
...
3. **LLM 不替代 block**：缺关键阶段图/文由 **规则拒绝提交**（**限商家冷启动等路径**），不由模型「补写」；**用户授权** 路径写入 geo 快照但不 block。
...
```text
商家：按节点传图 + note（自由描述）
  → [用户授权] 车主 completed 即可提交（不因 GEO block）
  → [冷启动] 提交公开审核
  → [规则 block] 关键阶段证据是否齐全（仅非用户授权路径）
  → 脱敏完成（用户路径：已脱敏素材入 payload；失败/无图不拦提交）
  → [阶段 A] 节点聚合 → 模板生成初稿（article_status: ready）
  → [阶段 C] 异步 LLM 文本优化 → 建议稿（generationSource: llm_v1, riskChecked: false）
  → 运营台 diff 审核 → 批准（用户授权：合规闸门，非留档质量）
...
| ID | 任务 | 涉及文件 | 优先级 | 状态 | 备注 |
| ---: | --- | --- | ---: | ---: | --- |
| GEO-CITE-D01 | 多模态调用 | `backend/src/services/case-geo-vision.service.js` | P2 | [~] | **规则模板稿已上线**；多模态识图 API（脱敏 URL）待接 |
| GEO-CITE-D02 | 并入 LLM 任务 | `case-geo-llm.service.js` | P2 | [x] | 与阶段 C 同 diff 面板扩展「图说」Tab |
| GEO-CITE-D03 | H5 展示 | `h5/shared/case-render.js` | P2 | [x] | caption/alt 优先采纳稿 |
...
| GEO-CITE-D06 | Vision API 接入 | `case-geo-vision.service.js` | P2 | [ ] | `GEO_VISION_*` + 多模态 endpoint；仅脱敏图 |
...
| [`10_案例信任元数据开发计划.md`](./10_案例信任元数据开发计划.md) | trustMeta 专项 |
| [`11_GEO高阶聚合开发计划.md`](./11_GEO高阶聚合开发计划.md) | 高阶聚合专项 |
| [`.cursor/rules/zhejian-development.mdc`](../../.cursor/rules/zhejian-development.mdc) | 开发前读设计规范与 PRD |

---

## 15. 阶段 F · 案例信任元数据（P0 · 2026-07-13）

> **详表**：[`10_案例信任元数据开发计划.md`](./10_案例信任元数据开发计划.md) · 任务 ID `GEO-TRUST-*`

**目标**：相册隐私改造后，以授权/审核/版本等 trustMeta 补位单案信任；不改快照正文。

| 阶段 | 范围 | 优先级 | 状态 |
| --- | --- | ---: | ---: |
| F | trustMeta 生成 + H5/Feed/Schema | P0 | [ ] |

**与 CITE 关系**：trustMeta 挂提炼层；**禁止** LLM 改写；与节点聚合（阶段 A）独立。
...
| 2026-06-18 | V1.2 | C/E 验收；D 规则图说上线；D06 vision API 单列待接 |
| 2026-07-13 | V1.3 | 新增阶段 F trustMeta；对接 `10_案例信任元数据开发计划` |
