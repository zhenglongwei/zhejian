...
| 阶段 | 名称 | 优先级 | 依赖 | 产出 |
| --- | --- | ---: | --- | --- |
| **A** | 专题入库（去 mock）+ 运营 CRUD | P0 | 无 | 真库驱动 `/topic/` |
| **B** | 页内 FAQ + FAQPage Schema | P0 | A | AI 可引用的问答块 |
| **C** | 案例↔专题矩阵 + 自动推荐 | P1 | A、[`06`](./06_GEO案例引用优化开发计划.md) | 内链密度 |
| **D** | 意图模板批量生成 + 首批 30 专题 | P1 | A、B | 种子内容网 |
| **E** | 车型+服务轻量专题 | P2 | C | 覆盖 `DS-C-06` 子集 |
| **F** | llms.txt / 专题 Feed | P2 | A | 可选发现层 |

**推荐顺序**：`A → B → D`（首批上线）→ `C` → `E/F`。
...
| 2 | **GEO-TOPIC A～B～D**：有专题答案页 |
| 3 | [`06`](./06_GEO案例引用优化开发计划.md) C + **GEO-TOPIC C**：矩阵串通 |
| 4 | [`07`](./07_GEO引用观测开发计划.md) B～C：用数据验效果 |
