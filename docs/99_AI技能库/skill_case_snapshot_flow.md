...
| 文档 | 用途 |
| --- | --- |
| [`19_商家发布案例上网口径.md`](../04_维修过程相册/19_商家发布案例上网口径.md) | **公开权真源**（商家发布；过审即上；平台不代通知） |
...
1. **商家**制造留档并 **生成/发布** 公开案例；**用户**可看相册、公开后可撤下；**平台**只做合法合规（案例审），不代通知车主。
2. **确认并完工** → 商家锁定；**生成案例后**异步脱敏 → 脱敏结束进人审；**车主全程可看服务相册**。
3. **公开生效**（人审通过）→ **CaseSnapshot** 整包冻结；不再等车主点发布，也不等通知窗口。
4. **公示后快照一字不改**；FAQ/GEO/SEO/专题/聚合 FAQ 属 **GeoEnrichmentLayer**，可随时改。
5. **下架** → 公开下线；**不**解锁商家。
6. **案例审驳回** → **解锁商家**改相册再完工再生成；车主相册仍可看。
7. 生成案例时 **不可加新图**。公开权真源：`19_商家发布案例上网口径.md`。
8. 打开案例预览即自动润色（确认完工前；失败回退规则稿）；平台不在公示后替商家改快照。
9. **相册与案例文稿分模块**：审的是文稿不是相册。
