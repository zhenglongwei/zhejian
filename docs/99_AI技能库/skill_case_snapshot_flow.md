...
1. **商家**制造留档；**用户**决定公示/撤回；**平台**只做合法合规（案例审）。
2. **确认并完工** → 商家锁定 + 异步脱敏 → 脱敏结束进人审；**车主全程可看服务相册**（案例审不挡相册）。
3. **用户首次发布** → **CaseSnapshot** 整包冻结（含当时 title/body/nodes）；发布不再复审。
4. **公示后快照一字不改**；FAQ/GEO/SEO/专题/聚合 FAQ 属 **GeoEnrichmentLayer**，可随时改。
5. **再发布** → `snapshotVersion++`（案例审已通过则不强制重审）。
6. **案例审驳回** → **解锁商家**改相册+案例稿再送审；车主相册仍可看。
7. **撤回发布** → 公开下线；**不**解锁商家。
8. AI 润色仅商家在确认完工前发起；平台不在公示后替商家改快照。
9. **相册与案例文稿分模块**：审的是文稿不是相册；曝光/分享规则未定前不以案例审拦截相册查看。
