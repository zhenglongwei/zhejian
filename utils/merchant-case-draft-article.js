/**
 * 车主案例稿 → 可粘贴成文的图文导出（HTML）
 * 图片仅使用脱敏 URL；供「复制图文文章」页使用。
 */
...
/** 小程序预览用：章节 + 脱敏图列表 */
function buildDraftArticleSectionsView(draft = {}, options = {}) {
  const publicBaseUrl = options.publicBaseUrl || ''
  const sections = collectSectionBodies(draft)
  const mediaMap = mediaBySection(draft, publicBaseUrl)
  const usedKeys = new Set()
  const views = sections.map((sec) => {
    usedKeys.add(sec.key)
    return {
      key: sec.key || sec.title || 'section',
      title: sec.title || '正文',
      body: sec.body || '',
      images: mediaMap[sec.key] || [],
    }
  })
...
/**
 * @returns {{ title: string, html: string, plain: string }}
 */
