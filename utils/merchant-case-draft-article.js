/**
 * 车主案例稿 → 可粘贴成文的图文导出（HTML / Markdown / 纯文本）
 * 图片仅使用脱敏 URL；供预览页「复制文案」与剪贴板页使用。
 */
...
/**
 * @returns {{ title: string, html: string, markdown: string, plain: string }}
 */
function buildDraftArticleExport(draft = {}, options = {}) {
  ...
  sections.forEach((sec) => {
    ...
    images.forEach((img) => {
      htmlParts.push(`<img src="${escapeHtml(img.url)}" alt="${escapeHtml(img.caption || heading)}" style="${style.img}" />`)
      if (img.caption) {
...
/**
 * 剪贴板专用整页（web-view 内用 ClipboardItem 复制 text/html，便于粘贴成文）
 */
function buildDraftArticleClipboardPage(exportData = {}) {
  ...
  const plain = String(exportData.plain || '').replace(/\\/g, '\\\\').replace(/`/g, '\\`')
  return `<!DOCTYPE html>
<html lang="zh-CN">
<head>
  <meta charset="utf-8" />
